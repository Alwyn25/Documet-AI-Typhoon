...
def run_test():
    ...
    try:
        ...
        # --- 3. Run Test Cases ---
        app_host = os.getenv("APP_HOST", "127.0.0.1").replace("0.0.0.0", "127.0.0.1")
        ...
        print("\n--- Test Case 1: Successful Workflow ---")
        success_payload = {"raw_file_ref": "/path/to/invoice.pdf", "user_id": "test_user_1"}
        response = None
        try:
        ...
        print("\n--- Test Case 2: Anomaly Workflow ---")
        anomaly_payload = {"raw_file_ref": "/path/to/invoice.pdf", "user_id": "test_user_2"}
        response = None
        try:
            ...
            # The agent is now configured to create this specific flag for this user
            assert "HIGH_VALUE_INVOICE" in response_json.get("final_state", "")
            print("  - ✅ PASSED")
        except Exception as e:
            print(f"  - ❌ FAILED: {e}")
            if response is not None: print(f"    Response body: {response.text}")
